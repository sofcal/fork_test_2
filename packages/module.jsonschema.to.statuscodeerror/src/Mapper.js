...
class Mapper {
    ...
    static toStatusCodeError(result, Type, obj) {
        ...
        return StatusCodeError.Create(Mapper.toStatusCodeErrorItems(result, Type, obj), 400);
    }
}
...
const getPropertyNames = (err) => {
    ...
    // we need to store all parent objects so that our custom mappings can decide which level they want to use
    while (splitIndex > 1) {
        splitIndex -= 1;
