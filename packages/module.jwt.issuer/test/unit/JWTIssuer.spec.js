...
describe('@sage/sfab-s2s-jwt-issuer.JWTIssuer', function() {
    ...
    const defaultData = [{
        status: 'primary',
        privateKey: 'primary_private',
        publicKey: 'primary_public',
        createdAt: 0,
        kid: 'primary_kid'
    }, {
        status: 'secondary',
        privateKey: 'secondary_private',
        publicKey: 'secondary_public',
        createdAt: 0,
        kid: 'secondary_kid'
    }];

    const serviceId = 'sage.servicefabric.banking';
    const newCertDelay = 100;
    ...
    describe('generate', () => {
        it('should include an exp claim which contains the expiration timestamp of the token', () => {
            ...
            sandbox.stub(defaultData[0], 'privateKey').value(primaryKey);
        ...
        it('should delay usage of a new certificate for X seconds; where X is configurable', () => {
            ...
            sandbox.stub(defaultData[0], 'createdAt').value(Math.floor(now / 1000) - 1);

            sandbox.stub(defaultData[1], 'privateKey').value(primaryKey);
            sandbox.stub(cache, 'getData').resolves(defaultData);
            sandbox.useFakeTimers(now);
