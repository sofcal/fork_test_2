...
describe('runonce-incident-bankAccountsByProduct.handler', function() {
    let sandbox;
    ...
    it('should create an S3 service', (done) => {
        sandbox.stub(dummyLoader, 'load').resolves(config);
        sandbox.stub(ParameterStoreStaticLoader, 'Create').returns(dummyLoader);
        sandbox.stub(DB, 'Create').returns(db);
        sandbox.stub(S3, 'Create').returns(s3);
        ...
        handler.run(event, context, () => { })
            .then(() => {
                should(S3.Create.callCount).eql(1);
                should(S3.Create.calledWithExactly(
                    { bucket: 's3-bucket' }
                )).eql(true);
    ...
    it('should call impl.run', (done) => {
        ...
        handler.run(event, context, () => {
            try {
                ...
                should(impl.run.getCall(0).args).eql([
                    event, config, { db, s3 }
                ]);
