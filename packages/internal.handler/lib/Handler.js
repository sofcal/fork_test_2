...
const {
  StatusCodeError
} = require('internal-status-code-error'); // const DB = require('internal-services-db');
...
class Handler {
  ...
  run(event, context, callback) {
    ...
    return Promise.resolve(undefined).then(() => {
      ...
      event.logger = RequestLogger.Create({
      ...
      return getParams({
        env,
        region
      }, self, event.logger).then(params => {
        return self.populateServices(services, {
          env,
          region,
          params
        }, event.logger).then(() => {
          return connectDB(services, params, event.logger, self.dbName).then(() => params);
        });
      });
    }).then(params => this.impl(event, params, services)).then(ret => {
      const response = {
        statusCode: 200,
      ...
      event.logger.info({
        function: func,
        log: 'sending success response: 200'
      });
      ...
      const statusCodeError = StatusCodeError.is(err) ? err : StatusCodeError.CreateFromSpecs([ErrorSpecs.internalServer], ErrorSpecs.internalServer.statusCode);
      const response = statusCodeError.toDiagnoses();
      const status = statusCodeError.statusCode;
      event.logger.info({
        function: func,
        log: `sending failure response: ${status}`,
        response
      });
      callback(err.failLambda ? err : null, {
        statusCode: status,
...
const populateServicesImpl = Promise.method((self, services, {
  env,
  region,
  params
}, logger) => {
  ...
  const {
    dbName
  } = self; // add any additional services that are created by the serviceLoader for the lambda

  Object.assign(services, self.serviceLoader({
    env,
    region,
    params,
    dbName
...
const getParams = ({
  ...
  const {
    keys
  } = self; // eslint-disable-line no-shadow
  ...
  const loader = ParameterStoreStaticLoader.Create({
    keys,
    paramPrefix,
    env: {
      region
  ...
  return loader.load(params).then(() => {
    const retrievedCount = Object.keys(params).length;
    logger.info({
      function: func,
      log: 'finished retrieving param-store keys',
...
const connectDB = (services, params, logger, dbName) => {
  ...
  return services.db.connect(dbName).then(db => {
    logger.info({
      function: func,
      log: 'ended'
    });
...
const disconnectDB = Promise.method((services, logger) => {
  ...
  return services.db.disconnect().then(() => {
    logger.info({
      function: func,
      log: 'ended'
    });
