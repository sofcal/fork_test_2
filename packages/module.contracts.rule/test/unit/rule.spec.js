    ...
    describe('contract.Rule', () => {
        ...
        describe('Rule.filter', () => {
            ...
            it('should filter out secret & irrelevant info', (done) => {
                const filteredRule = Rule.filter(uut);

                should(filteredRule.uuid).eql('2a867f21-cf12-48f2-914f-5a741e719b27');
                should.not.exist(filteredRule.productId);
                should.not.exist(filteredRule.globalRuleId);
                should(filteredRule.ruleName).eql('New rule');
                should(filteredRule.ruleRank).eql(1);
                should.not.exist(filteredRule.targetType);
                should(filteredRule.ruleType).eql('User');
                should(filteredRule.status).eql('active');

                should(filteredRule.ruleConditions).be.an.Array().of.length(1);
                should(filteredRule.ruleConditions[0].ruleField).eql('transactionAmount');
                should(filteredRule.ruleConditions[0].ruleOperation).eql('gt');
                should(filteredRule.ruleConditions[0].ruleCriteria).eql('10');

                should(filteredRule.ruleActions).be.an.Array().of.length(1);
                should(filteredRule.ruleActions[0].splitPercentage).eql(100);
                should(filteredRule.ruleActions[0].accountantNarrative).eql('New rule narrative');

                should(filteredRule.ruleActions[0].accountsPostings).be.an.Array().of.length(1);
                should(filteredRule.ruleActions[0].accountsPostings[0].type).eql('customer');
                should(filteredRule.ruleActions[0].accountsPostings[0].code).eql('CUST1');

                should.not.exist(filteredRule.ruleCounts);

                should(filteredRule.ruleAdditionalFields).be.an.Array().of.length(1);
                should(filteredRule.ruleAdditionalFields[0].name).eql('auto-post');
                should(filteredRule.ruleAdditionalFields[0].value).eql('true');

                should.not.exist(filteredRule.created);
                should.not.exist(filteredRule.updated);
                should.not.exist(filteredRule.etag);
            ...
            it('should remove non-readonly properties from the old rule if using put', function (done) {
                try {
                    ...
                    should(extended).eql({
                        ...
                        targetType: 'Transaction',
                        productId: 'dc8c78bc-cff2-43ea-9f5f-f0cc0f4df1dc',
                        ruleCounts: { success: 3, fail: 1, applied: 2 },
                        globalRuleId: null
                    });
