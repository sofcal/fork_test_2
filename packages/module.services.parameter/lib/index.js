...
const getParametersImpl = Promise.method((self, params) => {
  const mapResponse = response => {
    const returnParams = {}; // for loop used for performance reasons
...
const setParameterImpl = Promise.method((self, {
  name: Name,
  value: Value,
  type: Type,
  keyId: KeyId,
  ...
  return self.ssm.putParameter(options).promise().then(data => data);
});
