...
Promise.resolve()
    ...
    .then(client => {
        dbClient = client;
        return client.db('bank_db')
    })
    .then(db => {
        db.collection('Product').remove({});
        db.collection('Bank').remove({});
        db.collection('Organisation').remove({});
        db.collection('Company').remove({});
        db.collection('BankAccount').remove({});
        db.collection('Transaction').remove({});
        db.collection('Unresolved').remove({});

        db.collection('Bank').insert(banks);
        db.collection('Product').insert(products);
        db.collection('Organisation').insert(organisations);
        db.collection('Company').insert(companies);
        db.collection('BankAccount').insert(bankAccounts);
        db.collection('Transaction').insert(transactions);
        db.collection('Unresolved').insert(unresolved);
        return db;
    })
    .then(() => dbClient.close())
    .catch(err => {
