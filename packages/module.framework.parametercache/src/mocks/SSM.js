...
class SSM {
    ...
    getParametersByPath(...args) {
        return getParametersByPathImpl(this, ...args);
    }

    getParameters(...args) {
        return getParametersImpl(this, ...args);
    }
}

const getParametersByPathImpl = Promise.method((self, options) => {
    const prefix = options.Path;
    return getParams(self, options, (key) => (!prefix || key.indexOf(prefix) === 0));
});

const getParametersImpl = Promise.method((self, options) => {
