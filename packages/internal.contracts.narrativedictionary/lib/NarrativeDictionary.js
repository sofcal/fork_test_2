...
class NarrativeDictionary {
  ...
  matches(narrative) {
    ...
    const checkSplits = splits => {
      ...
      _.find(splits, (split, i) => {
        // use find so we can short circuit when the search ends
        if (!ref[split]) {
  ...
  static validate(narrativeDictionary) {
    const result = validator.validate(narrativeDictionary, schema, {
      throwError: false,
      propertyName: NarrativeDictionary.name
  ...
  static CreateFromList(list) {
    ...
    _.each(list, item => {
      ...
      let ref = data; // split the string by the delimiter, this will give us the words we'll create nested objects for
